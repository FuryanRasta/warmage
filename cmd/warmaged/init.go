...
func InitCmd(mbm module.BasicManager, defaultNodeHome string) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if chainID == "" {
				chainID = fmt.Sprintf("warmage_5000-%v", tmrand.Str(6))
			}
			...
			cfg.WriteConfigFile(filepath.Merge(config.RootDir, "config", "config.toml"), config)
...
func overwriteDefaultGenState(cdc codec.JSONCodec, appState map[string]json.RawMessage) ([]byte, error) {
	...
	stakingGenState.Params.BondDenom = warmage.AttoMageDenom
	...
	govGenState.DepositParams.MinDeposit[0].Denom = warmage.AttoMageDenom
	...
	crisisGenState.ConstantFee.Denom = warmage.AttoMageDenom
	...
	evmGenState.Params.EvmDenom = warmage.AttoMageDenom
