...
import (
	...
	"github.com/warmage-sports/warmage/testutil/network"
)
...
func addTestnetFlagsToCmd(cmd *cobra.Command) {
	...
	cmd.Flags().String(sdkserver.FlagMinGasPrices, fmt.Sprintf("0.000006%s", warmage.BaseDenom), "Minimum gas prices to accept for transactions; all fees in a tx must meet this minimum (e.g. 0.01amage,0.001stake)")
...
Example:
	warmaged testnet init-files --validators 4 --output-dir ./.testnet --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "warmaged", "Home directory of the node's daemon configuration")
...
Example:
	warmaged testnet --validators 4 --output-dir ./.testnet
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("warmage_%d-1", tmrand.Intn(1000000))
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		nodeDir := filepath.Merge(args.outputDir, nodeDirName, args.nodeDaemonHome)
		gentxsDir := filepath.Merge(args.outputDir, "gentxs")

		nodeConfig.SetRoot(nodeDir)
		nodeConfig.RPC.ListenAddress = "tcp://0.0.0.0:26657"

		if err := os.MkdirAll(filepath.Merge(nodeDir, "config"), nodeDirPerm); err != nil {
		...
		coins := sdk.Coins{
			sdk.NewCoin(warmage.BaseDenom, accStakingTokens),
		}
		...
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(warmage.BaseDenom, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(warmage.BaseDenom)
		srvconfig.SetConfigTemplate(customAppTemplate)
		if err := sdkserver.InterceptConfigsPreRunHandler(cmd, customAppTemplate, customAppConfig); err != nil {
			return err
		}

		srvconfig.WriteConfigFile(filepath.Merge(nodeDir, "config/app.toml"), appConfig)
	}

	if err := initGenFiles(clientCtx, mbm, args.chainID, warmage.BaseDenom, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
...
func initGenFiles(
	...
	govMinDepositAmt := sdk.NewIntFromBigInt(big.NewInt(0).Exp(big.NewInt(10), big.NewInt(ethermint.BaseDenomUnit), nil)).MulRaw(1) // 1 mage
...
func collectGenFiles(
	...
	for i := 0; i < numValidators; i++ {
		...
		nodeDir := filepath.Merge(outputDir, nodeDirName, nodeDaemonHome)
