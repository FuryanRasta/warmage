...
func GetCmdCreateSuper() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(
			"%s tx guardian add-super --chain-id=<chain-id> --from=<key-name> --fees=0.3mage --address=<added address> --description=<name>",
			version.AppName,
		),
...
func GetCmdDeleteSuper() *cobra.Command {
	cmd := &cobra.Command{
		...
		Example: fmt.Sprintf(
			"%s tx guardian delete-super --chain-id=<chain-id> --from=<key-name> --fees=0.3mage --address=<deleted address>",
			version.AppName,
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
