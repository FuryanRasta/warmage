...
func (b *Base) lastTimeRewardAvailable(ctx sdk.Context, rewardDenom string) uint64 {
	return warmage.Min(uint64(ctx.BlockTime().Unix()), b.GetReward(ctx, rewardDenom).FinishTime)
}
...
func (b *Base) rewardPerTicket(ctx sdk.Context, rewardDenom string) sdk.Int {
	...
	timeFromLast := b.lastTimeRewardAvailable(ctx, rewardDenom) - warmage.Min(reward.LastUpdateTime, reward.FinishTime)
...
func (b *Base) calculateRewardPerTicket(ctx sdk.Context, rewardDenom string, timestamp0, timestamp1, startTimestamp uint64, totalAmount sdk.Int) (rewardPerTicket sdk.Int, endTimestamp uint64) {
	reward := b.GetReward(ctx, rewardDenom)
	endTimestamp = warmage.Max(timestamp1, startTimestamp)
	duration := warmage.Min(endTimestamp, reward.FinishTime) - warmage.Min(warmage.Max(timestamp0, startTimestamp), reward.FinishTime)
...
func (b *Base) updateRewardPerTicket(ctx sdk.Context, rewardDenom string) {
	...
	if point.Amount.IsPositive() {
		rewardPerTicket, _ := b.calculateRewardPerTicket(ctx, rewardDenom, warmage.Max(point.Timestamp, startTimestamp), b.lastTimeRewardAvailable(ctx, rewardDenom), startTimestamp, point.Amount)
		rewardCumulativePerTicket = rewardCumulativePerTicket.Add(rewardPerTicket)
...
func (b *Base) userReward(ctx sdk.Context, rewardDenom string, veID uint64) sdk.Int {
	...
	startTimestamp := warmage.Max(userReward.LastClaimTime, b.GetRewardCheckpoint(ctx, rewardDenom, FirstEpoch).Timestamp)
