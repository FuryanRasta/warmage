...
import (
	...
	"github.com/warmage-sports/warmage/types"
	wartypes "github.com/warmage-sports/warmage/types"
...
func (suite *KeeperTestSuite) TestKeeper_GetDenomMetaData() {
	...
	var (
		t        = suite.T()
		k        = suite.app.BankKeeper
		denom    = types.MicroUSFDenom
		base     = denom
		display  = base[1:]
		uusfMeta = banktypes.Metadata{
	...
	require.Equal(t, uusfMeta.Description, meta.Description)
	require.Equal(t, uusfMeta.Base, meta.Base)
	require.Equal(t, uusfMeta.Display, meta.Display)
	require.Equal(t, uusfMeta.Name, meta.Name)
	require.Equal(t, uusfMeta.Symbol, meta.Symbol)
	require.Equal(t, uusfMeta.DenomUnits[0], meta.DenomUnits[0])
	require.Equal(t, uusfMeta.DenomUnits[1], meta.DenomUnits[1])
}
