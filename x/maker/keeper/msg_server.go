...
func (m msgServer) MintBySwap(c context.Context, msg *types.MsgMintBySwap) (*types.MsgMintBySwapResponse, error) {
	...
	backingIn, mageIn, mintOut, mintFee, err := m.Keeper.calculateMintBySwapOut(ctx, msg.BackingInMax, msg.MageInMax, msg.FullBacking)
	if err != nil {
		return nil, err
	}
	...
	poolBacking.MageBurned = poolBacking.MageBurned.Add(mageIn)

	totalBacking.WarMinted = totalBacking.WarMinted.Add(mintTotal)
	totalBacking.MageBurned = totalBacking.MageBurned.Add(mageIn)

	m.Keeper.SetPoolBacking(ctx, poolBacking)
	m.Keeper.SetTotalBacking(ctx, totalBacking)

	// take backing and mage coin
	err = m.Keeper.bankKeeper.SendCoinsFromAccountToModule(ctx, sender, types.ModuleName, sdk.NewCoins(backingIn, mageIn))
	if err != nil {
		return nil, err
	}
	// burn mage
	if mageIn.IsPositive() {
		err = m.Keeper.bankKeeper.BurnCoins(ctx, types.ModuleName, sdk.NewCoins(mageIn))
		if err != nil {
			return nil, err
		}
	}
	...
	ctx.EventManager().EmitEvents(sdk.Events{
		sdk.NewEvent(types.EventTypeMintBySwap,
			sdk.NewAttribute(types.AttributeKeyCoinIn, sdk.NewCoins(backingIn, mageIn).String()),
...
func (m msgServer) RedeemCollateral(c context.Context, msg *types.MsgRedeemCollateral) (*types.MsgRedeemCollateralResponse, error) {
	...
	if accColl.WarDebt.Amount.ToDec().Mul(warmage.MicroUSFTarget).GT(maxDebtInUSD) {
...
func (m msgServer) LiquidateCollateral(c context.Context, msg *types.MsgLiquidateCollateral) (*types.MsgLiquidateCollateralResponse, error) {
	...
	if accColl.WarDebt.Amount.ToDec().Mul(warmage.MicroUSFTarget).LT(liquidationValue) {
	...
	repayIn := sdk.NewCoin(warmage.MicroUSFDenom, msg.Collateral.Amount.ToDec().Sub(liquidationFee).Mul(collateralPrice).Quo(warmage.MicroUSFTarget).TruncateInt())

	if msg.RepayInMax.IsLT(repayIn) {
		return nil, sdkerrors.Wrap(types.ErrWarSlippage, "")
	}

	// repay for debtor as much as possible, and repay interest first
	repayDebt := sdk.NewCoin(warmage.MicroUSFDenom, sdk.MinInt(accColl.WarDebt.Amount, repayIn.Amount))
	warMinted := repayIn.Sub(repayDebt)

	repayInterest := sdk.NewCoin(warmage.MicroUSFDenom, sdk.MinInt(accColl.LastInterest.Amount, repayDebt.Amount))
	accColl.LastInterest = accColl.LastInterest.Sub(repayInterest)
	...
	if warMinted.IsPositive() {
		err = m.Keeper.bankKeeper.SendCoinsFromModuleToAccount(ctx, types.ModuleName, debtor, sdk.NewCoins(warMinted))
		if err != nil {
			return nil, err
		}
	}
	...
	if !found {
		if len(allowNewAccount) > 0 && allowNewAccount[0] {
			acc = types.AccountCollateral{
				...
				WarDebt:             sdk.NewCoin(warmage.MicroUSFDenom, sdk.ZeroInt()),
