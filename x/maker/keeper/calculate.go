...
func (k Keeper) calculateMintBySwapIn(
	...
	mintFee = sdk.NewCoin(warmage.MicroUSFDenom, sdk.ZeroInt())
	...
	mintTotalInUSD := mintTotal.Amount.ToDec().Mul(warmage.MicroUSFTarget)
	...
	if backingRatio.GTE(sdk.OneDec()) || fullBacking {
		...
		// full algorithmic
		mageIn.Amount = mintTotalInUSD.QuoRoundUp(magePrice).RoundInt()
	} else {
		// fractional
		backingIn.Amount = mintTotalInUSD.Mul(backingRatio).QuoRoundUp(backingPrice).RoundInt()
		mageIn.Amount = mintTotalInUSD.Mul(sdk.OneDec().Sub(backingRatio)).QuoRoundUp(magePrice).RoundInt()
	}
...
func (k Keeper) calculateMintBySwapOut(
	...
	if backingRatio.GTE(sdk.OneDec()) || fullBacking {
		...
		mageIn.Amount = mageInMax.Amount
	} else {
		// fractional
		...
		if backingInMax.IsPositive() && (mageInMax.IsZero() || max1.LTE(max2)) {
			...
			backingIn.Amount = backingInMax.Amount
			mageIn.Amount = mintTotalInUSD.Mul(sdk.OneDec().Sub(backingRatio)).QuoRoundUp(magePrice).RoundInt()
			if mageInMax.IsPositive() && mageInMax.IsLT(mageIn) {
				mageIn.Amount = mageInMax.Amount
			}
		} else {
			mintTotalInUSD = max2
	...
	mintTotal := sdk.NewCoin(warmage.MicroUSFDenom, mintTotalInUSD.Quo(warmage.MicroUSFTarget).TruncateInt())
...
func (k Keeper) calculateBurnBySwapIn(
	...
	burnInValue := burnActualInUSD.Quo(warmage.MicroUSFTarget).Quo(sdk.OneDec().Sub(burnFeeRate))
	burnFeeValue := burnInValue.Mul(burnFeeRate)
	burnIn = sdk.NewCoin(warmage.MicroUSFDenom, burnInValue.RoundInt())
	burnFee = sdk.NewCoin(warmage.MicroUSFDenom, burnFeeValue.RoundInt())
	return
}
...
func (k Keeper) calculateBurnBySwapOut(
	...
	burnActualInUSD := burnActual.Amount.ToDec().Mul(warmage.MicroUSFTarget)
...
func (k Keeper) calculateBuyBackingIn(
	...
	mageInValue := backingOutTotal.Amount.ToDec().Mul(backingPrice)

	if mageInValue.GT(excessBackingValue.ToDec()) {
	...
	mageIn = sdk.NewCoin(warmage.AttoMageDenom, mageInValue.Quo(magePrice).RoundInt())
...
func (k Keeper) calculateBuyBackingOut(
	...
	mageInValue := mageIn.Amount.ToDec().Mul(magePrice)
	if mageInValue.GT(excessBackingValue.ToDec()) {
		err = sdkerrors.Wrap(types.ErrBackingCoinInsufficient, "")
		return
	}

	backingOutTotal := sdk.NewCoin(backingDenom, mageInValue.Quo(backingPrice).TruncateInt())
...
func (k Keeper) calculateMintByCollateral(
	...
	availableDebtMax := collateralValue.Mul(availableLTV).Quo(warmage.MicroUSFTarget).TruncateInt()
...
func (k Keeper) checkMintPriceLowerBound(ctx sdk.Context) error {
	...
	mintPriceLowerBound := warmage.MicroUSFTarget.Mul(sdk.OneDec().Add(k.MintPriceBias(ctx)))
	if warPrice.LT(mintPriceLowerBound) {
		return sdkerrors.Wrapf(types.ErrWarPriceTooLow, "%s price too low: %s", warmage.MicroUSFDenom, warPrice)
	}
	return nil
}
...
func (k Keeper) checkBurnPriceUpperBound(ctx sdk.Context) error {
	...
	burnPriceUpperBound := warmage.MicroUSFTarget.Mul(sdk.OneDec().Sub(k.BurnPriceBias(ctx)))
	if warPrice.GT(burnPriceUpperBound) {
		return sdkerrors.Wrapf(types.ErrWarPriceTooHigh, "%s price too high: %s", warmage.MicroUSFDenom, warPrice)
	}
	return nil
}
