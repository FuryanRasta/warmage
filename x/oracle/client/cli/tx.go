...
# Aggregate Prevote
$ warmaged tx oracle aggregate-prevote 1234 amage:1.234,uusf:0.99

where "amage,uusf" is the denominating currencies, and "1.234,0.99" is the exchange rates of these currencies in $uUSD from the voter's point of view.

If voting from a voting delegate, set "validator" to the address of the validator to vote on behalf of:
$ warmaged tx oracle aggregate-prevote 1234 amage:1.234,uusf:0.99 warvaloper1...
`),
...
$ warmaged tx oracle aggregate-vote 1234 amage:1.234,uusf:0.99
...
If voting from a voting delegate, set "validator" to the address of the validator to vote on behalf of:
$ warmaged tx oracle aggregate-vote 1234 amage:1.234,uusf:0.99 warvaloper1...
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
$ warmaged tx oracle set-feeder war1...

where "war1..." is the address you want to delegate your voting rights to.
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
