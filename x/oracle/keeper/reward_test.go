...
func TestRewardBallotWinners(t *testing.T) {
	...
	// Prepare reward pool
	givingAmt := sdk.NewCoins(sdk.NewInt64Coin(warmage.AttoMageDenom, 30000000), sdk.NewInt64Coin(warmage.MicroUSFDenom, 40000000))
	...
	require.Equal(t, sdk.NewDecFromInt(givingAmt.AmountOf(warmage.AttoMageDenom)).QuoInt64(votePeriodsPerWindow).QuoInt64(3).TruncateInt(),
		outstandingRewards.AmountOf(warmage.AttoMageDenom))
	require.Equal(t, sdk.NewDecFromInt(givingAmt.AmountOf(warmage.MicroUSFDenom)).QuoInt64(votePeriodsPerWindow).QuoInt64(3).TruncateInt(),
		outstandingRewards.AmountOf(warmage.MicroUSFDenom))
	...
	require.Equal(t, sdk.NewDecFromInt(givingAmt.AmountOf(warmage.AttoMageDenom)).QuoInt64(votePeriodsPerWindow).QuoInt64(3).MulInt64(2).TruncateInt(),
		outstandingRewards1.AmountOf(warmage.AttoMageDenom))
	require.Equal(t, sdk.NewDecFromInt(givingAmt.AmountOf(warmage.MicroUSFDenom)).QuoInt64(votePeriodsPerWindow).QuoInt64(3).MulInt64(2).TruncateInt(),
		outstandingRewards1.AmountOf(warmage.MicroUSFDenom))
}
