...
func TestToCrossRate(t *testing.T) {
	...
	for _, data := range data {
		...
		if !data.base.IsZero() {
			pbBase = append(pbBase, types.NewVoteForTally(data.base, warmage.AttoMageDenom, valAddr, 100))
		}

		pbQuote = append(pbQuote, types.NewVoteForTally(data.quote, warmage.AttoMageDenom, valAddr, 100))

		if !data.base.IsZero() && !data.quote.IsZero() {
			cb = append(cb, types.NewVoteForTally(data.base.Quo(data.quote), warmage.AttoMageDenom, valAddr, 100))
		} else {
			cb = append(cb, types.NewVoteForTally(sdk.ZeroDec(), warmage.AttoMageDenom, valAddr, 0))
		}
	}
