...
Keys allows you to manage your local tendermint keystore (wallets) for mage.

## Available Commands

| Name                            | Description                                                                                      |
| ------------------------------- | ------------------------------------------------------------------------------------------------ |
| [add](#mage-keys-add)           | Add an encrypted private key (either newly generated or recovered), encrypt it, and save to disk |
| [delete](#mage-keys-delete)     | Delete the given key                                                                             |
| [export](#mage-keys-export)     | Export private keys                                                                              |
| [import](#mage-keys-import)     | Import private keys into the local keybase                                                       |
| [list](#mage-keys-list)         | List all keys                                                                                    |
| [migrate](#mage-keys-migrate)   | Migrate keys from the legacy (db-based) Keybase                                                  |
| [mnemonic](#mage-keys-mnemonic) | Compute the bip39 mnemonic for some input entropy                                                |
| [parse](#mage-keys-parse)       | Parse address from hex to bech32 and vice versa                                                  |
| [show](#mage-keys-show)         | Retrieve key information by name or address                                                      |
...
```bash
mage keys add <multisig-keyname> --multisig-threshold=2 --multisig=<signer-keyname-1>,<signer-keyname-2>,<signer-keyname-3>
```
...
Offline key can be created by "mage keys add --pubkey".
:::

How to use multisig key to sign and broadcast a transaction,  please refer to [multisign](tx.md#mage-tx-multisign)
...
```bash
mage keys delete <key-name> [flags]
```
...
```bash
mage keys export <key-name> [flags]
```

### Export keystore

```bash
mage keys export Mykey --output-file=<path-to-keystore>
```
...
```bash
mage keys import <name> <keyfile> [flags]
```
...
```bash
mage keys parse <hex-or-bech32-address> [flags]
```
