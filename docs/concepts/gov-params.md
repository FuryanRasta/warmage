...
In MAGEhub, there are some special parameters that can be modified through on-chain governance.
All the MAGE holders may participate in the on-chain governance. If the community is not satisfied with certain modifiable parameters, it is available to submit a [parameter-change](../features/governance.md#usage-scenario-of-parameter-change) proposal, and the params will be changed online automatically when the proposal passes.
...
| key                 | Description                                      | Range                                                    | Current                                                                                                        |
| ------------------- | ------------------------------------------------ | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------- |
| `gov/depositparams` | Related parameters of the deposit mortgage phase | max_deposit_period:(0, 9223372036854775807]              | {"min_deposit": [{"denom": "umage", "amount": "1000000000"}], "max_deposit_period": "86400s" }                 |
...
| `mint/MintDenom` | Denom of the token mintable |          | umage   |
...
| `service/MinDeposit`           | Minimum deposit amount                                      | amount: (0, +∞)           | [{"denom": "umage","amount": "5000000000"}] |
| `service/BaseDenom`            | Token denom that must be used for deposits                  |                           | umage                                       |
...
| `staking/BondDenom`         | Bond denom                                                      |                          | umage    |
...
| `token/IssueTokenBaseFee` | Base token for issuing tokens     | amount: (0, +∞) | {"denom": "mage","amount": "60000"} |
