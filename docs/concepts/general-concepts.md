...
Delegators are MAGE holders who cannot, or do not want to run a validator themselves. MAGE holders can delegate MAGE to a validator and obtain a part of their revenue in exchange. They can earn as much as the validators and only need to pay some commission.
...
- **Block Inflation**
  ...
  Block Inflation exists to incentivize MAGE holders to stake. The more staked MAGE tokens are, more secure the network become(Read more about [Staking](../features/staking.md)).

  Block Inflation will be [distributed every block](../features/mint.md). [Inflation rate](../features/mint.md) in MAGEhub for the first year will be 4%.  **This ration could be adjusted by `parameter-change` proposals**.
  In this way, loose MAGE will devalue year by year.

- **Block Proposer Reward**

  In MAGEhub, the probability for being a proposer is proportional to the validator's bonded tokens. If one proposed block is finalized, the proposer gets extra rewards for it.

- **Fee**

  Each transaction needs a [fee](fee.md#fee) for compensating validators' work[Gas](fee.md#gas). These fees can be paid with MAGE and may later in any tokens which are whitelisted by the MAGEhub's governance. Fees are distributed to validators in proportion to their stake. A minimum fee/gas ration is set in MAGEhub.
...
The following formulas are based on the current [MAGEhub Mainnet Params](gov-params.md).

#### Annual Rewards (ignore proposer rewards and fees)

- **AnnualInflation =** `Base * InflationRate` (aka 2 billion * 4% = 80 million mage)
...
- **BlockInflation =** `AnnualInflation / (365*24*60*12)` (aka 12.68 mage)
