...
### How to Become an MAGEhub Validator
...
- `unbonding`:
  ...
  - Validator is out of the top 100 bonded and become a Candidate, the validator can delegate more MAGE to himself to be in top 100, then he will get bonded automatically.
...
- It is generated when the node is created by `mage init`
...
- Query the associated hex address
  ...
  ```bash
  mage status | jq .validator_info.address
  ```
...
This key is created from `mage` and used to sign transactions. Application keys are associated with a public key prefixed by `iap` and an address prefixed by `iaa`. Both are derived from account keys generated by `mage keys add`.
...
If you are using the software sign (which is the default signing method of tendermint), your [Tendermint Key](#tendermint-key) is located in `<mage-home>/config/priv_validator.json`. The easiest way is to backup the whole config folder.
...
### Is there a minimum amount of MAGE that must be delegated to be an active (=bonded) validator

The minimum amount is `1 mage`.
...
### How often will a validator be chosen to propose the next block? Does it go up with the quantity of bonded MAGE

The validator that is selected to propose the next block is called proposer. Each proposer is selected deterministically, and the frequency of being chosen is proportional to the voting power (i.e. amount of bonded MAGE) of the validator. For example, if the total bonded stake across all validators is 100 MAGE and a validator's total stake is 10, then this validator will propose ~10% of the blocks.
...
Revenue received by a validator's pool is split between the validator and their delegators. The validator can apply a commission on the part of the revenue that goes to their delegators. This commission is set as a percentage. Each validator is free to set their initial commission, maximum daily commission change rate and maximum commission. The MAGE Hub enforces the parameter that each validator sets. Only the commission rate can change after the validator is created.
...
4. [Edit your validator](../cli-client/staking.md#mage-tx-staking-edit-validator) and specify `--identity=<the_16_digit_string>`
...
First, if `mage` is not running, start it up again:

```bash
mage start
```

Wait for your full node to catch up to the latest block. Then, you can [unjail your validator](../cli-client/slashing.md#mage-tx-slashing-unjail) if you have been jailed over the [DowntimeJailDuration](gov-params.md#parameters-in-slashing)
...
### My `mage` crashes because of `too many open files`

The default number of files Linux can open (per-process) is `1024`. `mage` is known to open more than `1024` files. This causes the process to crash. A quick fix is to run `ulimit -n 4096` (increase the number of open files allowed, only effective for the current session) and then restart the process with `mage start`. If you are using `systemd` or another process manager to launch `mage` this may require some configuration at that level.

- A sample `systemd` file to fix this issue is below:

    ```toml
    # /etc/systemd/system/mage.service
    [Unit]
    Description=MAGE Hub Node
    ...
    ExecStart=/home/ubuntu/go/bin/mage start
    Restart=on-failure
    RestartSec=3
    LimitNOFILE=65535
...
- From [Explorer](https://warmage.iobscan.io/#/staking), you can find the `Consensus Pubkey` which you declared for your validator in the Validator Detail page.

- Check the `Consensus Pubkey` which you are now using via `mage tendermint show-validator --home=<mage-home>`
...
- Stop the node
- Replace the current `<mage-home>/config/priv_validator.json` with the one you backed up
- Confirm the `Consensus Pubkey` is correct via `mage tendermint show-validator --home=<mage-home>`
- Start the node
- Check `voting_power` should now be greater than 0: `mage status`

#### What if I lost my Tendermint Key

That means you have **Lost Your Validator Forever!** You can only create a new one and [redelegate](../cli-client/staking.md#mage-tx-staking-redelegate) all of your shares to the new Validator.
