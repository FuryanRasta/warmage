...
The `grpc.Server` is a concrete gRPC server, which spawns and serves any gRPC requests. This server can be configured inside `~/.mage/config/app.toml`:
...
An overview of all available gRPC endpoints shipped with the MAGEhub is [Protobuf documention](./proto-docs.md).
