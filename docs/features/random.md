...
This specification describes the usage and scope of random numbers on MAGEhub. This feature is currently in beta and please assess the risk yourself before using.

## Concepts

Currently, MAGEhub provides two random number generation methods: PRNG and TRNG.
...
On the basis of PRNG, the external random number factor `Oracle Seed` is introduced using the oracle machine method, implemented through MAGEhub Service.
...
- [Request Random Number](../cli-client/rand.md#mage-tx-random-request-random)
- [Query Random Number](../cli-client/rand.md#mage-query-random-random)
- [Query Random Queue](../cli-client/rand.md#mage-query-random-queue)
