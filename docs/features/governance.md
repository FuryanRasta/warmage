...
- **Vote:** Once deposit reaches a certain value (`MinDeposit`), proposal is
  confirmed and vote opens. Bonded Mage holders can then send `TxGovVote`
...
Any Mage holder, whether bonded or unbonded, can submit proposals by sending a
...
Cosmos Hub, participants are bonded Mage holders. Unbonded Mage holders and
...
- _participant_ bonded or unbonded Mage to said validator after proposal
  entered voting period.

- _participant_ became validator after proposal entered voting period.

This does not prevent _participant_ to vote with Mage bonded to other
validators. For example, if a _participant_ bonded some Mage to validator A
before a proposal entered voting period and other Mage to validator B after
...
Proposals can be accepted before the end of the voting period if they meet a special condition. Namely, if the ratio of `Yes` votes to `InitTotalVotingPower`exceeds 2:3, the proposal will be immediately accepted, even if the `Voting period` is not finished. `InitTotalVotingPower` is the total voting power of all bonded Mage holders at the moment when the vote opens.
...
mage tx gov submit-proposal param-change proposal.json --from=<key-name> --fees=0.3mage --chain-id=warmage
```
...
mage tx gov submit-proposal community-pool-spend proposal.json --from=<key-name> --fees=0.3mage --chain-id=warmage
```
