...
Voting power is a consensus concept. MAGEhub is a Byzantine-fault-tolerant POS blockchain network. During the consensus process, a set of validators will vote the proposal block. If a validator thinks the proposal block is valid, it will vote `yes`, otherwise, it will vote nil. The votes from different validator don't have the same weight. The weight of a vote is called the voting power of the corresponding validator.

### Validator

Validator is a full MAGEhub node. If its voting power is zero, it is just a normal full node or a validator candidate. Once its voting power is positive, then it is a real validator.
...
So, it is recommended that validator operator reserve at least 1 mage while unbonding tokens.**
:::
...
```bash
mage q staking validator iva14n9md3sq9xwscs96za8n85m0j9y2yu3cagxgke
```
...
```bash
mage tx staking edit-validator --from=<key-name> --chain-id=warmage --fees=0.3mage --commission-rate=0.15 --moniker=<new-name>
```
...
```bash
mage tx staking delegate iva14n9md3sq9xwscs96za8n85m0j9y2yu3cagxgke 1000mage --chain-id=warmage --from=<key-name> --fees=0.3mage
```

- Unbond tokens from a validator

```bash
mage tx staking unbond iva14n9md3sq9xwscs96za8n85m0j9y2yu3cagxgke 1000mage --chain-id=warmage --from=<key-name> --fees=0.3mage
```
  
- Redelegate tokens to another validator

```bash
mage tx staking redelegate iva14n9md3sq9xwscs96za8n85m0j9y2yu3cagxgke iva1l2rsakp388kuv9k8qzq6lrm9taddae7fpx59wm 100mage --from mykey --chain-id=warmage --from=<key-name> --fees=0.3mage
```
