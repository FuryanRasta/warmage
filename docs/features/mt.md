...
```plain
mage tx mt issue --name=<denom-name> --from=<sender-address>
```
...
```plain
mage tx mt mint <denom-id> --amount=<amount> --data=<data> --from=<sender-address> --recipient=<recipient-address>
```
...
```plain
mage tx mt mint <denom-id> --mt-id=<mt-id> --amount=<amount> --from=<sender-address> --recipient=<recipient-address>
```
...
```plain
mage tx mt edit <denom-id> <mt-id> --data=<data> --from=<sender-address>
```
...
```plain
mage tx mt transfer <sender> <recipient> <denom-id> <mt-id> <amount>
```
...
```plain
mage tx mt burn <denom-id> <mt-id> <amount> --from=<sender-address>
```
...
```plain
mage query mt supply <denom-id> <mt-id>
```
...
```plain
mage query mt balances <owner> <denom-id>
```
...
```plain
mage query mt token <denom-id> <mt-id>
```
