...
The module combines `service` to achieve decentralized injection from trusted Oracles such as `Chainlink` to MAGEhub Oracle. Each data collection task is called a feed, and its underlying implementation depends on the `service` module. Feed life cycle
...
```bash
mage tx oracle start test-feed \
    --chain-id="warmage-test" \
    --from=node0 \
    --fees=0.3mage \
    --broadcast-mode=block
...
```bash
mage tx oracle pause test-feed \
    --chain-id="warmage-test" \
    --from=node0 \
    --fees=0.3mage \
    --broadcast-mode=block
