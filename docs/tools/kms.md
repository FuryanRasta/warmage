...
Initialize configuration files for MAGEhub

```bash
tmkms init -n warmage /path/to/kms/home
```

## Configuration

To enable KMS, you need to edit the `priv_validator_laddr` in your `<mage-home>/config/config.toml` file first. e.g.:
...
Then, downLoad [priv_validator_state.json example](https://github.com/warmage-sports/warmage/blob/master/docs/tools/priv_validator_state.json) and modify all field values to match your `<mage-home>/data/priv_validator_state.json` values.
...
- Edit `addr` to point to your `mage` instance(note: no need to specify the connection id, just like tcp://localhost:26658).
- Adjust `chain_id` to match your `<mage-home>/config/genesis.json` settings.
...
If you want to import an existing MAGEhub private_key:

```bash
tmkms yubihsm keys import <mage_home>/config/priv_validator.json -i <id> -t json -c /path/to/kms/home/tmkms.toml
```
