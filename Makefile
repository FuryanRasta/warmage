...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
WARMAGE_BINARY = warmaged
WARMAGE_DIR = warmage
BUILDDIR ?= $(CURDIR)/build
BUILD_BRIDGING_DIR ?= $(CURDIR)/build-bridging
SIMAPP = ./app
HTTPS_GIT := https://github.com/warmage-sports/warmage.git
...
build-docker:
	...
	mkdir -p ./build/
	$(DOCKER) cp warmage:/usr/bin/warmaged ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/warmage-sports/warmage/types"
	godoc -http=:6060
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/warmage-sports/warmage
.PHONY: format
...
localnet-start: localnet-stop build-linux localnet-build
	@if ! [ -f $(BUILDDIR)/node0/warmaged/config/genesis.json ]; \
...
localnet-unsafe-reset:
	docker-compose down
ifeq ($(OS),Windows_NT)
	@docker run --rm -v $(CURDIR)\localnet-setup\node0\warmaged:warmage\Z warmaged/node "./warmaged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)\localnet-setup\node1\warmaged:warmage\Z warmaged/node "./warmaged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)\localnet-setup\node2\warmaged:warmage\Z warmaged/node "./warmaged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)\localnet-setup\node3\warmaged:warmage\Z warmaged/node "./warmaged unsafe-reset-all --home=/warmage"
else
	@docker run --rm -v $(CURDIR)/localnet-setup/node0/warmaged:/warmage:Z warmaged/node "./warmaged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)/localnet-setup/node1/warmaged:/warmage:Z warmaged/node "./warmaged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)/localnet-setup/node2/warmaged:/warmage:Z warmaged/node "./warmaged unsafe-reset-all --home=/warmage"
	@docker run --rm -v $(CURDIR)/localnet-setup/node3/warmaged:/warmage:Z warmaged/node "./warmaged unsafe-reset-all --home=/warmage"
endif
...
bridging-localnet-start: bridging-localnet-stop build-linux localnet-build
	...
	@if ! [ -f $(BUILD_BRIDGING_DIR)/node0/warmaged/config/genesis.json ]; \
...
PACKAGE_NAME:=github.com/warmage-sports/warmage
