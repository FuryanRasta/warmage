...
build: go.sum
ifeq ($(OS),Windows_NT)
	go build $(BUILD_FLAGS) -o build/mage.exe ./cmd/mage
else
	go build $(BUILD_FLAGS) -o build/mage ./cmd/mage
endif
...
build-all-binary: go.sum
	LEDGER_ENABLED=false GOOS=linux GOARCH=amd64 go build $(BUILD_FLAGS) CGO_ENABLED=1 -o build/mage-linux-amd64 ./cmd/mage
	LEDGER_ENABLED=false GOOS=linux GOARCH=arm64 go build $(BUILD_FLAGS) CGO_ENABLED=1 -o build/mage-linux-arm64 ./cmd/mage
	LEDGER_ENABLED=false GOOS=windows GOARCH=amd64 go build $(BUILD_FLAGS) CGO_ENABLED=1 -o build/mage-windows-amd64.exe ./cmd/mage
...
install: go.sum
	go install $(BUILD_FLAGS) ./cmd/mage
...
draw-deps:
	...
	@goviz -i ./cmd/mage -d 2 | dot -Tpng -o dependency-graph.png
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./lite/statik/statik.go" -not -path "*.pb.go" | xargs goimports -w -local github.com/warmage-sports/warmage
...
testnet-init:
	@if ! [ -f build/nodecluster/node0/mage/config/genesis.json ]; then docker run --rm -v $(CURDIR)/build:/home warmage-sports/warmage mage testnet --v 4 --output-dir /home/nodecluster --chain-id warmage-test --keyring-backend test --starting-ip-address 192.168.10.2 ; fi
	@echo "To install jq command, please refer to this page: https://stedolan.github.io/jq/download/"
	@jq '.app_state.auth.accounts+= [{"@type":"/cosmos.auth.v1beta1.BaseAccount","address":"iaa1ljemm0yznz58qxxs8xyak7fashcfxf5lgl4zjx","pub_key":null,"account_number":"0","sequence":"0"}] | .app_state.bank.balances+= [{"address":"iaa1ljemm0yznz58qxxs8xyak7fashcfxf5lgl4zjx","coins":[{"denom":"umage","amount":"1000000000000"}]}]' build/nodecluster/node0/mage/config/genesis.json > build/genesis_temp.json ;
	@sudo cp build/genesis_temp.json build/nodecluster/node0/mage/config/genesis.json
	@sudo cp build/genesis_temp.json build/nodecluster/node1/mage/config/genesis.json
	@sudo cp build/genesis_temp.json build/nodecluster/node2/mage/config/genesis.json
	@sudo cp build/genesis_temp.json build/nodecluster/node3/mage/config/genesis.json
	@rm build/genesis_temp.json
	@echo "Faucet address: iaa1ljemm0yznz58qxxs8xyak7fashcfxf5lgl4zjx" ;
	@echo "Faucet coin amount: 1000000000000umage"
